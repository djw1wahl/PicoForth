...
//
void _dup (void)   { PushP = TopP; }                                                                               // dup ( a b c - a b c c )
void _drop(void)   { M.pidx--; }                                                                                   // drop ( a b c - a b )
void _swap(void)   { M.TMPA = TopP; TopP = M.pstack[M.pidx -1]; M.pstack[M.pidx -1] = M.TMPA; }                    // swap ( a b c - a c b )
void _over(void)   { M.TMPA = M.pstack[M.pidx -1]; PushP = M.TMPA; }                                               // over ( a b c - a b c b )
void _prot(void)   { M.TMPC = M.pstack[M.pidx -0]; M.TMPB = M.pstack[M.pidx -1]; M.TMPA = M.pstack[M.pidx -2]; \
                     M.pstack[M.pidx -0] = M.TMPA; M.pstack[M.pidx -1] = M.TMPC; M.pstack[M.pidx -2] = M.TMPB; }   //  rot ( a b c - b c a ) 
void _nrot(void)   { M.TMPC = M.pstack[M.pidx -0]; M.TMPB = M.pstack[M.pidx -1]; M.TMPA = M.pstack[M.pidx -2]; \
                     M.pstack[M.pidx -0] = M.TMPB; M.pstack[M.pidx -1] = M.TMPA; M.pstack[M.pidx -2] = M.TMPC; }   // -rot ( a b c - c a b )
void _2drop(void)  { M.pidx -= 2; }                                                                                // 2drop( a b c - a )
...
void _add(void)    { M.pstack[M.pidx-1] += TopP; M.pidx--; }                                                       // ( x y - x+y )
void _sub(void)    { M.pstack[M.pidx-1] -= TopP; M.pidx--; }                                                       // ( x y - x-y )
void _mul(void)    { M.pstack[M.pidx-1] *= TopP; M.pidx--; }                                                       // ( x y - x*y )
...
void _equal(void)  { if(M.pstack[M.pidx-1] == M.pstack[M.pidx]) \
                         { M.pidx--; PushP = true; } \
                     else{ M.pidx--; PushP = false; } }                                                            // ( x y - x==y | x!=y )    
